...
export BASE_MODEL='/root/shared-nvme/models/Qwen2.5-3B'
export EXPERIMENT_NAME=nq-search-r1-ppo-qwen2.5-3b-em
export WANDB_MODE=offline
export WANDB_DIR=verl_checkpoints/$EXPERIMENT_NAME/wandb
...
# Note: FSDP normalizes PPO batch sizes by DP world size (here 8 GPUs),
# so global ppo_*_micro_batch_size must be >= 8 to avoid becoming 0.
...
nohup env PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.optim.type=badam \
    +actor_rollout_ref.actor.optim.switch_block_every=100 \
    +actor_rollout_ref.actor.optim.switch_mode=random \
    +actor_rollout_ref.actor.optim.verbose=1 \
    ...
    +critic.optim.type=badam \
    +critic.optim.switch_block_every=100 \
    +critic.optim.switch_mode=random \
    +critic.optim.verbose=1 \
...
TRAIN_PID=$!
echo "PPO training started in background. PID=$TRAIN_PID, log=$LOG_FILE"
